...
public:
	...
	KvsClient(vector<UserRoutingThread> routing_threads, string ip,
			...
			routing_threads_(routing_threads), ut_(UserThread(ip, tid)), context_(
					zmq::context_t(1)), socket_cache_(
					SocketCache(&context_, ZMQ_PUSH)), key_address_puller_(
					zmq::socket_t(context_, ZMQ_PULL)), response_puller_(
					zmq::socket_t(context_, ZMQ_PULL)), log_(
					spdlog::basic_logger_mt("client_log", "client_log.txt",
							true)), timeout_(timeout) {
		// initialize logger
...
public:
	...
	string vput_async(const vector<Key>& keys, const vector<string>& payloads,
			int size, LatticeType lattice_type) {
		KeyRequest request;
	...
	string vget_async(const vector<Key>& keys, int size) {
		...
		/*
		 for (int i = 0; i < size; i++) {
		 if (pending_get_response_map_.find(keys[i])
		 != pending_get_response_map_.end()) {
		 return "";
		 }
		 }
	...
	vector<KeyResponse> receive_async() {
		...
		if (pollitems_[0].revents & ZMQ_POLLIN) {
			...
			if (pending_request_map_.find(key) != pending_request_map_.end()) {
				if (response.error() == AnnaError::NO_SERVERS) {
					...
					pending_request_map_[key].first =
		...
		if (pollitems_[1].revents & ZMQ_POLLIN) {
			...
			if (response.type() == RequestType::GET) {
				...
				if (pending_put_response_map_.find(key)
						...
						&& pending_put_response_map_[key].find(
					...
					if (check_tuple(response.tuples(0))) {
						...
						try_request(
								pending_put_response_map_[key][response.response_id()].request_);
					} else {
						// error no == 0
						result.push_back(response);
						pending_put_response_map_[key].erase(
								response.response_id());
		...
		for (const auto& pair : pending_request_map_) {
			if (std::chrono::duration_cast < std::chrono::milliseconds
					> (std::chrono::system_clock::now() - pair.second.first).count()
					> timeout_) {
		...
		for (const auto& pair : pending_get_response_map_) {
			if (std::chrono::duration_cast < std::chrono::milliseconds
					> (std::chrono::system_clock::now() - pair.second.tp_).count()
					> timeout_) {
		...
		for (const auto& key_map_pair : pending_put_response_map_) {
			for (const auto& id_map_pair : pending_put_response_map_[key_map_pair.first]) {
				if (std::chrono::duration_cast < std::chrono::milliseconds
						> (std::chrono::system_clock::now()
								- pending_put_response_map_[key_map_pair.first][id_map_pair.first].tp_).count()
						> timeout_) {
					result.push_back(
							generate_bad_response(id_map_pair.second.request_));
					to_remove_put[key_map_pair.first].insert(id_map_pair.first);
					invalidate_cache_for_worker(
							id_map_pair.second.worker_addr_);
				}
			}
		}
...
private:
	...
	void try_request(KeyRequest& request) {
		...
		Address worker = get_worker_thread(key); // xyao, read from a address pool
		if (worker.length() == 0) {
		...
		int num_tuples = request.tuples_size();
		for(int i=0; i<num_tuples; i++){
			request.mutable_tuples(i)->set_address_cache_size(key_address_cache_[key].size());
		}
		...
		if (request.type() == RequestType::GET) {
			...
			if (pending_put_response_map_[key].find(request.request_id())
				...
				pending_put_response_map_[key][request.request_id()].tp_ =
						std::chrono::system_clock::now();
				pending_put_response_map_[key][request.request_id()].request_ =
						request;
			}
	...
	Address get_routing_thread() {
		return routing_threads_[rand_r(&seed_) % routing_threads_.size()].key_address_connect_address();
	}
